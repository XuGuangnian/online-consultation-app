<template>
  <div>
    ...
    <!-- 1. 语法糖写法 -->
    <Son v-model="count" v-model:show="show" />
    <!-- 2. 完整写法
    $event:
    1. 如果是原生js事件，它的值是=》事件对象
    2. 如果是组件的自定义事件，它的值是=》子传父的data
    -->
    <!-- <Son :model-value="count" @update:model-value="count = $event" /> -->
    <!-- <Son
      :model-value="count"
...
/**
 * vue3的v-model语法糖
 * 背景：父子通信，因为遵循单向数据流，父传子的变量，只能通过
 * 子传父自定义事件，去修改父组件中的变量=》使用上有点麻烦
 * 解决：可以使用v-model语法糖解决子改父麻烦问题，本质还是父子通信
 * v-model效果：
 * v-model绑定父的变量
 * 1. 父的变量变化=》子变化
 * 2. 子变化=》父的变量变化
 * 原理总结：
 * v-model === :modelValue + @update:modelValue （一个)
